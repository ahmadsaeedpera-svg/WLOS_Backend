...
public sealed class LifeOsIntegrationTests(DatabaseFixture fixture)
{
    ...
    public async Task An_unbuilt_engine_says_so_rather_than_returning_silence()
    {
        ...
        try
        {
            ...
            /*  recommendationResolution used to be named here, then
                predictionResolution. Both are built now, so this names stages
                that genuinely are not — the assertion is that the pipeline
                still admits its gaps, not that any particular engine is
                missing. When the last of these is built, replace them rather
                than deleting the test: a pipeline reporting no unavailable
                stages should be true, not merely unasserted. */
            unavailable.Should().Contain(t => t.Stage == "conversationContextResolution");
